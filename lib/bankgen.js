...
const GEN_SYSTEM = `You design short problem-solving challenges for children, aligned to the Skills Builder Problem Solving steps. You always write at the requested reading level, keep scenarios kind and age-appropriate, and never include anything frightening, violent, sexual, commercial, or unsafe. You return strictly valid JSON and nothing else.`;
...
Reading level: about a ${readingAge}-year-old — short sentences, simple everyday words. Theme every scenario around ${interest}, friendly and playful.
For any type that asks the child to commit to an answer (evaluate, decision, mystery, information, dilemma), also write a "curveball": one short follow-up that introduces a new fact making the answer less obvious, in very simple words, never revealing the right answer.
...
function baseUser(track, form) {
  ...
  const other =
    form === "endline"
      ...
      : "";
  return `Write one staged warm-up problem for a child, reading level about ${readingAge}. Track steps: ${t.steps}.${other}
It has one short scenario and exactly 3 parts, each part exercising one of the track's steps in order.
Return ONLY JSON: {"scenario":"1-2 short sentences","stages":[{"step":<n>,"label":"2-3 word label","q":"one short question"},{"step":<n>,"label":"...","q":"..."},{"step":<n>,"label":"...","q":"..."}]}`;
}
